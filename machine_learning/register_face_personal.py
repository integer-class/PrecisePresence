...
def get_db_connection():
    connection = mysql.connector.connect(
        host="phpmyadmin.test",  # Replace with your actual host
        user="root",
        password="",
        database="dev"
    )
...
@app.post("/register_face_personal/")
async def register_face(user_id: int, file: UploadFile = File(...)):
    ...
    face, prob = mtcnn(img, return_prob=True)
    if face is None or prob < 0.90:
        return {"filename": file.filename, "message": "No face detected with sufficient confidence."}
    ...
    try:
        ...
        connection.commit()
        return {"filename": image_name, "message": "Embedding saved successfully"}
    except mysql.connector.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        cursor.close()
...
async def face_match(file: UploadFile = File(...), threshold: float = 0.7):
    ...
    face, prob = mtcnn(img, return_prob=True)
    if face is None or prob < 0.90:
        return {"status": "error", "message": "No face detected with sufficient confidence."}
